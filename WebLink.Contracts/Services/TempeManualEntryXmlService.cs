...
{
    public class TempeManualEntryXmlService :  IManualEntryService 
    {
        ...
        public TempeManualEntryXmlService(IOrderPoolRepository repo,
                                IUserData userData,
                                IAppConfig config,
                       ...
                       )
        {
            ...
            var dto = new ManualOrderData()
            {
                ...
                return await SendOrder(rq,dto, details, articles);
          


        }

        private List<TempeOrderLine> GetOrderDetail(DataEntryRq rq, string username)
        {
            return  JsonConvert.DeserializeObject<List<TempeOrderLine>>(rq.RowJsonData); 
        }
        ...
        private async Task<OperationResult> SendOrder(DataEntryRq rq, ManualOrderData dto, List<TempeOrderLine> details,
                      List<OrderArticle> articles)
        {
                    ...
                    foreach(var article in articles)
                    {
                        foreach (var detail in details)
                        {
                            writer.WriteLine(
                                ...
                                + detail.Modelo + delimiterChar
                                + detail.Calidad + delimiterChar 
                                + detail.ColorCode + delimiterChar
                                + detail.SizeCode + delimiterChar
                                + detail.Equivalencia[0] + delimiterChar
                                + detail.Equivalencia[0] + delimiterChar
                                + detail.Equivalencia[0] + delimiterChar
                                + detail.Equivalencia[0] + delimiterChar
                                + detail.Color + delimiterChar
                                + detail.DescCampana + delimiterChar
                                + detail.DescCadena + delimiterChar
                                + detail.DescPaisOrigen + delimiterChar
                                + detail.Medidas + delimiterChar
                                + detail.CodigoBarras[0] + delimiterChar
                                + detail.CodigoBarras[0] + delimiterChar
                                + detail.FullFamily + delimiterChar
                                + detail.FullMadeIn + delimiterChar
                                + detail.FullCareInstructionsAllLangs + delimiterChar
                                + detail.FullCareInstructions 
                                + detail.FullCareInstructionsString
                                + detail.Importadores + delimiterChar
                                + detail.FullExceptions + delimiterChar
                                + detail.XmlPath + delimiterChar
                                + rq.ProviderReference + delimiterChar 
                                + article.ArticleCode + delimiterChar 
                                + rq.FreeText1 + delimiterChar 
                                + rq.FreeText2 + delimiterChar 
                                + rq.Price1 + delimiterChar
                                + rq.Price2 + delimiterChar
                                + rq.Price3 + delimiterChar
                                + rq.Price4 + delimiterChar 
                                + rq.Description + delimiterChar
                                + detail.Quantity + delimiterChar
                                + rq.Currency1 + delimiterChar 
                                + rq.Currency2 + delimiterChar 
                                + rq.Currency3 + delimiterChar
                                + rq.Currency4 + delimiterChar
                                + rq.CountryDestination + delimiterChar
