...
{
    [FriendlyName("Scalpers - Composition Text Plugin")]
    [Description("Concatenate Scalpers Composition Sections and Care Instructions.")]
    public class ScalpersCompoPlugin : IWizardCompositionPlugin
    {
        ...
        public ScalpersCompoPlugin(IOrderUtilService orderUtilService)
        {
                ...
                foreach (var c in composition)
                {
                    ...
                    for (var i = 0; i < c.Sections.Count; i++)
                    {
                        ...
                        if (i < MAX_SHOES_SECTIONS)
                            compositionData.Add(symbolShoe, string.Join(";", ShoesSymbols.Distinct()));
