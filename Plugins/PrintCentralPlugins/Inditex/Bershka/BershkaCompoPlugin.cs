...
{
    [FriendlyName("Inditex.Bershka(old) - Composition Text Plugin")]
    [Description("Inditex.Bershka(old) - Composition Plugin")]
    public class BershkaCompoPlugin : IWizardCompositionPlugin
    {
                    ...
                    if(compositionData != null)
                    {
                        ...
                        return new List<PluginCompoPreviewData>() {
                            new PluginCompoPreviewData()
                            {
                                ...
                                CompositionText = GetCompostionText( compositionData, composition), 
        ...
        private List<PluginCompositionTextPreviewData> GetCompostionText(Dictionary<string, string> compositionData, CompositionDefinition composition)
        {
        ...
        private void GenerateCompositionTextByCompoId(List<OrderPluginData> orderData, int id)
        {
            ...
            foreach(var od in orderData)
            {
                ...
                foreach(var compo in compositions)
                {
                    ...
                    //get list Fibers
                    listfibers = CreateFiberList(compo, od.FillingWeightId, od.FillingWeightText, od.ExceptionsLocation);
        ...
        public void CalculateArticle(string ArticleCode, CompositionDefinition compo, int compoIndex, OrderPluginData od, StringBuilder additionals, int allCompoPages, int allAdditionalPages)
        {
            ...
            foreach(var job in printerjob)
            {
                ...
                if(newarticle == null)
                {
                    ...
                    var title = $"Bershka Composition Article [{ArticleCode}_{compoCode}] not found";
                    ...
                    throw new Exception($"Bershka Composition Article [{ArticleCode}_{compoCode}] not found");
                }
        ...
        public void AdditionalByPage(CompositionDefinition compo, StringBuilder careInstructions, StringBuilder additionals, StringBuilder Symbols, Dictionary<string, string> compositionData, RibbonFace materialsFaces, int linenumber)
        {
            ...
            if(!FillCareInstructionsPagesOneByOne(addExc, materialsFaces, linenumber, out filledPages))
            {
                if(!FillCareInstructionsPagesByTranslations(addExcTable, materialsFaces, linenumber, out filledPages))
                {
                    throw new Exception("BershkaCompoPlugin ERROR - Can't set Additional text");
                }
        ...
        public List<CompositionTextDTO> CreateFiberList(WebLink.Contracts.Models.CompositionDefinition compo, int fillingWeightId, string fillingWeightText, int exceptionsLocation)
        {
            ...
            for(var i = 0; i < compo.Sections.Count; i++)
            {
                ...
                for(var f = 0; f < compo.Sections[i].Fibers.Count; f++)
                {
                    ...
                    list.Add(new CompositionTextDTO
                    {
                        Percent = compo.Sections[i].Fibers[f].Percentage + "%",
                        Text = fiberValue,
