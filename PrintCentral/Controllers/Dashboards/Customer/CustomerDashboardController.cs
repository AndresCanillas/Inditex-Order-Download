...
namespace PrintCentral.Controllers.Dashboards.Customer
{
    [Authorize]
    public class CustomerDashboardController : Controller
    {
        private readonly static object syncObj = new object();
        private static Dictionary<int, DashboardTask> Tasks = null;
        ...
        private async Task<Dictionary<int, DashboardTask>> GetCustomerDashboardTasks()
        {
            lock (syncObj)
            {
                if (Tasks != null)
            ...
            if (dashboardTasks != null)
            {
                ...
                lock (syncObj)
                {
                    ...
                    Tasks = new Dictionary<int, DashboardTask>();
                    foreach (var dasboardTask in dashboardTasks)
                    {
                        ...
                        if (wftask != null)
                            Tasks.Add(wftask.TaskID, dasboardTask);
                    }
                }
            }
        ...
        [HttpGet, Route("/Dashboards/Customer/GetCountersByTask")]
        public async Task<OperationResult> GetCountersByTask()
        {
            ...
            if (tasks != null)
            {
                ...
                {
                    CountersByTaskResult.Add(new TaskInfo(task.Key, task.Value.Description, 0, 0));
                }
                ...
                {
                    var t = CountersByTaskResult.FirstOrDefault(o => o.TaskName == tasks[counter.Value.TaskID].Description);
                    if(t != null)
                    {
        ...
        [HttpGet, Route("/Dashboards/Customer/GetCustomersList")]
        public OperationResult GetCustomersList()
        {
            ...
            if (!userData.UserRoles.Contains("SysAdmin"))
            {
        ...
        [HttpGet, Route("/Dashboards/Customer/GetCountersByTaskForUser/{userid}")]
        ...
        [HttpGet, Route("/Dashboards/Customer/GetItemsWithErrors")]
            ...
            [FromQuery] int page)
        {
            ...
            foreach (var item in ItemsWithErrors)
            {
                ...
                project = cache.ProjectCache.GetByID(item.ProjectID);
                if (project != null)
                {
                    brand = cache.BrandCache.GetByID(project.BrandID);
                    if (brand != null)
                    {
                        company = cache.CompanyCache.GetByID(brand.CompanyID);
                    }
                }
                ...
                itemErrorInfo.ProjectName = project != null ? project.Name : "";
                itemErrorInfo.BrandName = brand != null ? project.Name : "";
                itemErrorInfo.CompanyName = company != null ? company.Name : "";
        ...
        [HttpGet, Route("/Dashboards/Customer/GetUserTotalItemsAsync/{userid}")]
        public async Task<int> GetUserTotalItemsAsync(string userid)
        {
            ...
            if (projectsAssignedToUser.Count > 0)
            {
                ...
                foreach (var counter in counters)
                {   
                    var currentTask = tasks[counter.Value.TaskID];
                    var t = CountersByTaskResult.FirstOrDefault(o => o.TaskName == currentTask.Description && !currentTask.ExcludeFromCounter);
                    if (t != null)
                    {
                        t.Waiting = counter.Value.Counters.Waiting;
                        t.Rejected = counter.Value.Counters.Rejected;
        ...
        [HttpPost, Route("/Dashboards/Customer/{workflowid}/cancel")]
        ...
        [HttpPost, Route("/Dashboards/Customer/{workflowid}/retry")]
        ...
        private class DashboardTask
        {
            ...
            public bool ExcludeFromCounter { get; set; }        
        }
