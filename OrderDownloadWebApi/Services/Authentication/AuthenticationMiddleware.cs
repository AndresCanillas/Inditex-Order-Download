...
namespace OrderDownloadWebApi.Services.Authentication
{
    public static class AuthenticationMiddleware
    {
        ...
        public static void UsePrintAuthentication(this IApplicationBuilder app)
        {
            ...
            app.Use(async (context, next) =>
            {
                ...
                || context.Request.Path.Value.Contains("/cls")
                || context.Request.Path.Value.Contains("/anonymous"))
                    await next.Invoke();
                else
        ...
        {
            if(context.Request.Cookies.TryGetValue(config.GetValue<string>("DownloadServicesWeb.bearerTokenName", "bearerToken"), out var token))
            {
        ...
        private static async Task<bool> InitializeIdentityFromCredentials(HttpContext context, string credentials)
        {
            if(GetCredentials(credentials, out var userName, out var password))
            {
                ...
                var result = await userRepo.AuthenticateAsync(userName, password);
                if(result.Success)
                {
                    context.User = result.Principal;
                    context.Response.Cookies.Append(config.GetValue<string>("DownloadServicesWeb.bearerTokenName", "bearerToken"), tokenService.RegisterPrincipal(result.Principal));
